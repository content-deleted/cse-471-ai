(define (domain pacmanD)
    (:requirements :strips)
    (:types pacman loc)
    (:predicates (pacman-at ?p -pacman ?l -loc) (east ?x -loc ?y -loc) (west ?x -loc ?y -loc) (north ?x -loc ?y -loc) (south ?x -loc ?y -loc) (fruit-at ?x ))  ; fill rest of the predicates 
    ...
    (:action move-East
     ...
     (:action move-North
      ...
      (:action EatDot
        :parameters (?p -pacman ?x -loc)
        :precondition (and (pacman-at ?p ?x) (fruit-at ?x))
        :effect (and (not (fruit-at ?x )))
        )
)